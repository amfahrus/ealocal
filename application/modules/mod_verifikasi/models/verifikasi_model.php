...
class verifikasi_model extends CI_Model {
    ...
    public function getJurnal($idproyek, $limit, $offset, $sidx, $sord, $cari, $search = "false") {
        ...
        //$this->db->where("((position('RKS' in nobukti) = 1) or (position('HTS' in nobukti) = 1) or (position('PUS' in nobukti) = 1))", NULL,false);
        ...
        foreach ($query->result_array() as $row) {
			//if((strpos($row["nobukti"], 'RKS') or strpos($row["nobukti"], 'PUS') or strpos($row["nobukti"], 'HTS')) and $row["isapprove"] == 'FALSE'){
			//if(preg_match('/HTS/',$row["nobukti"])){
			//if((strpos($row["nobukti"], 'S')) !== false and $row["isapprove"] == 'FALSE'){
			...
			//}
			//die(var_dump(preg_match('/S/',$row["nobukti"])));
        }
        //die(print_r($temp_result));
        return $temp_result;
    }
	...
	public function deleteJurnal($nobukti){
        ...
        $this->dataset_db->insert_logs(
										array(
											'log_username' => $this->session->userdata('ba_username'),
											'log_node' => $_SERVER['REMOTE_ADDR'],
											'log_description' => 'User menghapus jurnal verifikasi',
